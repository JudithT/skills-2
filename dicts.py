...
seach_key ='b'
''
'bagon nosepass startly yamask kalob baltoy'
    ...
    for word in names:
        if word[0] in letter_map:
            letter_map[word[0]].append(word)
        else:
            letter_map[word[0]] = [word]
   
    result = ''
    result +=  names[0]
    lookup_char = names[0][len(result)-1]
    
    while lookup_char in letter_map:
        found = False
        for word in letter_map[lookup_char]:
            if word not in result:
                result = result + ' ' + word 
                lookup_char = word[len(word)-1]
                found = True
                break
        if not found:
            return result.split()
